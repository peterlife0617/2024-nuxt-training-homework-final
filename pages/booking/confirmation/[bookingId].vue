          ...
          >
            ...
            <section class="d-flex flex-column gap-6">
              <h3 class="d-flex align-items-center mb-6 text-neutral-80 fs-8 fs-md-6 fw-bold">
                <p class="mb-0">
                  {{ data?.roomId.name }}，{{ diffDays(data?.checkInDate, data?.checkOutDate) }}晚
                </p>
                <span
              ...
              <div class="text-neutral-80 fs-8 fs-md-7 fw-bold">
                <p class="title-deco mb-2">
                  入住：{{ formatDate(data?.checkInDate, 'M 月 D 日dddd') }}，15:00 可入住
                </p>
                <p
                  class="title-deco mb-0"
                >
                  退房：{{ formatDate(data?.checkOutDate, 'M 月 D 日dddd') }}，12:00 前退房
                </p>
              </div>

              <p class="mb-0 text-neutral-80 fs-8 fs-md-7 fw-bold">
                {{ formatCurrency(data?.roomId.price) }}
              </p>
            </section>
            ...
            <section>
              ...
              <ul class="d-flex flex-wrap row-gap-2 column-gap-10 p-6 mb-0 fs-8 fs-md-7 bg-neutral-0 border border-neutral-40 rounded-3 list-unstyled">
                <li v-for="(facility, index) in data?.roomId.facilityInfo" :key="index" class="flex-item d-flex gap-2">
                  <Icon
            ...
            <section>
              ...
              <ul class="d-flex flex-wrap row-gap-2 column-gap-10 p-6 mb-0 fs-8 fs-md-7 bg-neutral-0 border border-neutral-40 rounded-3 list-unstyled">
                <li v-for="(amenity, index) in data?.roomId.amenityInfo" :key="index" class="flex-item d-flex gap-2">
                  <Icon
                    class="fs-5 text-primary-100"
                    icon="material-symbols:check"
                  />
